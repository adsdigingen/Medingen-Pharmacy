...
export class PurchaseSuggestionWorker extends BaseWorker {
  ...
  protected async run(): Promise<void> {
    ...
    const lowStock = lowStockInventories.filter(
    ...
    this.logger.log(
      `Found ${lowStock.length} low stock product(s). Generating procurement suggestions...`,
    );

    for (const inv of lowStock) {
      const suggestQty = Math.max(
        50,
        inv.product.minStockLevel * 2 - inv.availableQty,
      );
