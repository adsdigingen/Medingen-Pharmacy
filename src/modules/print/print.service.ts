...
export class PrintService {
  ...
  async generateReceiptText(
    ...
    if (widthType === '150x95mm') {
      ...
      const timeStr = new Date(bill.createdAt).toLocaleTimeString('en-US', {
        hour: '2-digit',
        minute: '2-digit',
      });
      ...
      // 5. Totals block
      const goodsValLabel =
        'Goods Value:'.padEnd(20) + bill.totalAmount.toFixed(2).padStart(12);
      lines.push(padLeftRight(`  ${bill.netAmount.toFixed(2)}`, goodsValLabel));

      const discountPercentage =
        (bill.discountAmount / bill.totalAmount) * 100 || 0;
      const discLabel =
        `Discount:      ${discountPercentage.toFixed(1)} %`.padEnd(20) +
        bill.discountAmount.toFixed(2).padStart(12);
      ...
      const roundOffLabel =
        'Rounded Off:'.padEnd(20) + roundOff.toFixed(2).padStart(12);
      lines.push(padLeftRight('', roundOffLabel));

      const billAmtLabel =
        'Bill Amount:'.padEnd(20) + bill.netAmount.toFixed(2).padStart(12);
    ...
    if (widthType === '58mm') {
      ...
      bill.billItems.forEach((item) => {
        ...
        const expiryStr = new Date(item.batch.expiryDate)
    ...
    if (bill.discountAmount > 0) {
      lines.push(
        padLeftRight('Discount Amount:', `-₹${bill.discountAmount.toFixed(2)}`),
      );
    }
