...
export class AuditListener {
  ...
  async handleBillCreated(event: any) {
    await this.log(
      'BILLING',
      'CREATE',
      `Bill #${event.billNumber} created. Net amount: ${event.netAmount}`,
    );
  }
  ...
  async handleBillCancelled(event: any) {
    await this.log(
      'BILLING',
      'UPDATE',
      `Bill #${event.billNumber} was cancelled. Reason: ${event.reason}`,
    );
  }
  ...
  async handleSalesReturn(event: any) {
    await this.log(
      'BILLING',
      'CREATE',
      `Sales return logged for Bill #${event.billNumber}. Returned items: ${event.returnedQty}`,
    );
  }
  ...
  async handlePurchaseOrderCreated(event: any) {
    await this.log(
      'PURCHASES',
      'CREATE',
      `Purchase Order #${event.poNumber} created in DRAFT.`,
    );
  }
  ...
  async handlePurchaseOrderReceived(event: any) {
    await this.log(
      'PURCHASES',
      'UPDATE',
      `Purchase Order #${event.poNumber} received. Status: ${event.status}`,
    );
  }
  ...
  async handlePurchaseReturnCreated(event: any) {
    await this.log(
      'PURCHASES',
      'CREATE',
      `Purchase Return credit note logged for PO #${event.poNumber}.`,
    );
  }
  ...
  async handleStockAdjusted(event: any) {
    await this.log(
      'INVENTORY',
      'UPDATE',
      `Stock adjusted for batch ID: ${event.batchId}. Type: ${event.type}, Qty: ${event.quantity}, Reason: ${event.reason}`,
    );
  }
  ...
  async handleCustomerCreated(event: any) {
    await this.log(
      'CUSTOMERS',
      'CREATE',
      `Customer "${event.name}" (${event.mobile}) was registered.`,
    );
  }
  ...
  async handleProductCreated(event: any) {
    await this.log(
      'PRODUCTS',
      'CREATE',
      `Product "${event.name}" SKU: ${event.sku || 'N/A'} was added.`,
    );
  }
  ...
  async handleProductUpdated(event: any) {
    await this.log(
      'PRODUCTS',
      'UPDATE',
      `Product "${event.name}" was modified.`,
    );
  }
  ...
  async handleProductImportStarted(event: any) {
    await this.log(
      'PRODUCTS',
      'IMPORT',
      `Product import started for supplier: ${event.supplierName}. Total rows: ${event.totalRows}`,
    );
  }
  ...
  async handleProductImportCompleted(event: any) {
    await this.log(
      'PRODUCTS',
      'IMPORT',
      `Product import completed for supplier: ${event.supplierName}. Success: ${event.successCount}, Errors: ${event.errorCount}`,
    );
  }
  ...
  async handleProductImportFailed(event: any) {
    await this.log(
      'PRODUCTS',
      'IMPORT',
      `Product import failed for supplier: ${event.supplierName}. Error: ${event.error}`,
    );
  }
