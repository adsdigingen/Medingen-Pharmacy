...
export class ProductsService {
  ...
  private mapProductLatestBatchPricing(product: any) {
    ...
    if (activeBatches.length > 0) {
      ...
      const sortedAll = [...product.batches].sort(
        (a: any, b: any) =>
    ...
    const purchasePrice =
      targetBatch && targetBatch.purchasePrice > 0
        ? targetBatch.purchasePrice
        : product.purchasePrice || 0;
    const mrp =
      targetBatch && targetBatch.mrp > 0 ? targetBatch.mrp : product.mrp || 0;

    let offlineSellingPrice =
      targetBatch && targetBatch.sellingPrice > 0
        ? targetBatch.sellingPrice
        : product.offlineSellingPrice || product.sellingPrice || 0;
    let onlineSellingPrice =
      targetBatch && targetBatch.onlineSellingPrice > 0
        ? targetBatch.onlineSellingPrice
        : product.onlineSellingPrice || 0;
    ...
    if (
      ...
      offlineSellingPrice = roundOff
        ? Math.round(computedOffline)
        : parseFloat(computedOffline.toFixed(2));
    ...
    if (onlineSellingPrice === 0 && onlineAutoCalculate && purchasePrice > 0) {
      ...
      onlineSellingPrice = roundOff
        ? Math.round(computedOnline)
        : parseFloat(computedOnline.toFixed(2));
    ...
    return {
      ...
      onlineSellingPrice,
      gstPercentage:
        targetBatch && targetBatch.gstPercentage !== undefined
          ? targetBatch.gstPercentage
  ...
  private async getPricingDefaults() {
    try {
      ...
      console.warn(
        '[ProductsService] Failed to fetch system settings from database, returning static defaults:',
        e.message,
      );
      return {
        defaultOfflineMarkup: 50.0,
  ...
  private calculateProductPrices(
    ...
    if (!offlineAutoCalculate && purchasePrice > 0) {
      calculatedOfflineMarkup = parseFloat(
        (((offlinePrice - purchasePrice) / purchasePrice) * 100).toFixed(2),
      );
    }
    if (!onlineAutoCalculate && purchasePrice > 0) {
      calculatedOnlineMarkup = parseFloat(
        (((onlinePrice - purchasePrice) / purchasePrice) * 100).toFixed(2),
      );
    }
    ...
    if (purchasePrice > 0) {
      if (offlinePrice < purchasePrice) {
        throw new BadRequestException(
          'Offline Selling Price cannot be lower than Purchase Price.',
        );
      }
      if (onlinePrice < purchasePrice) {
        throw new BadRequestException(
          'Online Selling Price cannot be lower than Purchase Price.',
        );
      }
    }
    if (mrp > 0) {
  ...
  async create(createProductDto: CreateProductDto) {
    ...
    const offlineMarkup =
      createProductDto.offlineMarkup ?? defaults.defaultOfflineMarkup;
    const onlineMarkup =
      createProductDto.onlineMarkup ?? defaults.defaultOnlineMarkup;
    ...
    const result = await this.repo.create({
      ...
      retailDiscount:
        createProductDto.retailDiscount ?? defaults.defaultRetailDiscount,
      roundOff,
    ...
    if (result) {
      try {
        ...
        console.error(
          '[ProductsService] Failed to create default inventory for product:',
          err.message,
        );
      }
  ...
  async findAll(query: {
    ...
    const mappedItems = items.map((item) =>
      this.mapProductLatestBatchPricing(item),
    );
  ...
  async update(
      ...
      for (const field of forbiddenFields) {
        if (dto[field] !== undefined && dto[field] !== prod[field]) {
          throw new ForbiddenException(
            `Pharmacists are not authorized to modify the pricing field: ${field}`,
          );
        }
      }
    }
    ...
    const offlineMarkup =
      updateProductDto.offlineMarkup !== undefined
        ? updateProductDto.offlineMarkup
        : product.offlineMarkup;
    const onlineMarkup =
      updateProductDto.onlineMarkup !== undefined
        ? updateProductDto.onlineMarkup
        : product.onlineMarkup;

    const offlineAutoCalculate =
      updateProductDto.offlineAutoCalculate !== undefined
        ? updateProductDto.offlineAutoCalculate
        : product.offlineAutoCalculate;
    const onlineAutoCalculate =
      updateProductDto.onlineAutoCalculate !== undefined
        ? updateProductDto.onlineAutoCalculate
        : product.onlineAutoCalculate;
    const roundOff =
      updateProductDto.roundOff !== undefined
        ? updateProductDto.roundOff
        : product.roundOff;

    const inputOfflinePrice =
      updateProductDto.offlineSellingPrice !== undefined
        ? updateProductDto.offlineSellingPrice
        : product.offlineSellingPrice;
    const inputOnlinePrice =
      updateProductDto.onlineSellingPrice !== undefined
        ? updateProductDto.onlineSellingPrice
        : product.onlineSellingPrice;
    ...
    const result = await this.repo.update(id, {
      name:
        updateProductDto.name !== undefined
          ...
          : product.name,
      genericName:
        updateProductDto.genericName !== undefined
          ? updateProductDto.genericName
          : product.genericName,
      brandName:
        updateProductDto.brandName !== undefined
      ...
      categoryId:
        updateProductDto.categoryId !== undefined
          ...
          : product.categoryId,
      manufacturerId:
        updateProductDto.manufacturerId !== undefined
          ? updateProductDto.manufacturerId || null
          : product.manufacturerId,
      supplierId:
        updateProductDto.supplierId !== undefined
          ? updateProductDto.supplierId || null
          : product.supplierId,
      hsnCode:
        updateProductDto.hsnCode !== undefined
          ? updateProductDto.hsnCode
          : product.hsnCode,
      gstPercentage:
        updateProductDto.gstPercentage !== undefined
          ? updateProductDto.gstPercentage
          : product.gstPercentage,
      purchasePrice,
      sellingPrice: calculated.offlineSellingPrice,
      mrp,
      ...
      hospitalPrice:
        updateProductDto.hospitalPrice !== undefined
          ? updateProductDto.hospitalPrice
          : product.hospitalPrice,
      memberPrice:
        updateProductDto.memberPrice !== undefined
          ? updateProductDto.memberPrice
          : product.memberPrice,
      specialOfferPrice:
        updateProductDto.specialOfferPrice !== undefined
          ? updateProductDto.specialOfferPrice
          : product.specialOfferPrice,
      retailDiscount:
        updateProductDto.retailDiscount !== undefined
          ? updateProductDto.retailDiscount
          : product.retailDiscount,
      roundOff,
      minStockLevel:
        updateProductDto.minStockLevel !== undefined
          ? updateProductDto.minStockLevel
          : product.minStockLevel,
      rackLocation:
        updateProductDto.rackLocation !== undefined
          ? updateProductDto.rackLocation
      ...
      description:
        updateProductDto.description !== undefined
          ...
          : product.description,
      drugSchedule:
        updateProductDto.drugSchedule !== undefined
          ? updateProductDto.drugSchedule
          : product.drugSchedule,
      medicineClassification:
        updateProductDto.medicineClassification !== undefined
          ? updateProductDto.medicineClassification
          : product.medicineClassification,
      prescriptionRequired:
        updateProductDto.prescriptionRequired !== undefined
          ? updateProductDto.prescriptionRequired
          : product.prescriptionRequired,
      coldChainRequired:
        updateProductDto.coldChainRequired !== undefined
          ? updateProductDto.coldChainRequired
          : product.coldChainRequired,
      controlledDrug:
        updateProductDto.controlledDrug !== undefined
          ? updateProductDto.controlledDrug
          : product.controlledDrug,
      highValueMedicine:
        updateProductDto.highValueMedicine !== undefined
          ? updateProductDto.highValueMedicine
          : product.highValueMedicine,
      storageCondition:
        updateProductDto.storageCondition !== undefined
          ? updateProductDto.storageCondition
  ...
  async importProducts(fileBuffer: Buffer) {
    ...
    for (let index = 0; index < rows.length; index++) {
      ...
      if (rowErrors.length > 0) {
        ...
        errors.push({
          row: rowNumber,
          medicineName: row['Medicine Name'] || 'Unknown',
          errors: rowErrors,
        });
        continue;
      }
      ...
      try {
        ...
        if (manufacturerName) {
          const cachedId = manufacturerCache.get(
        ...
        const offlineMarkup =
          row['Offline Markup'] !== undefined
            ? parseFloat(row['Offline Markup'])
            : defaults.defaultOfflineMarkup;
        const onlineMarkup =
          row['Online Markup'] !== undefined
            ? parseFloat(row['Online Markup'])
            : defaults.defaultOnlineMarkup;

        const roundOff =
          row['Round Off'] !== undefined
            ? String(row['Round Off']).toLowerCase() === 'yes' ||
              String(row['Round Off']).toLowerCase() === 'true'
            : true;

        const inputOfflinePrice =
          row['Offline Selling Price'] !== undefined
            ? parseFloat(row['Offline Selling Price'])
            : row['Selling Price'] !== undefined
              ? parseFloat(row['Selling Price'])
              : 0.0;
        const inputOnlinePrice =
          row['Online Selling Price'] !== undefined
            ? parseFloat(row['Online Selling Price'])
            : 0.0;
        ...
        const wholesalePrice =
          row['Wholesale Price'] !== undefined
            ? parseFloat(row['Wholesale Price'])
            : 0.0;
        const hospitalPrice =
          row['Hospital Price'] !== undefined
            ? parseFloat(row['Hospital Price'])
            : 0.0;
        const memberPrice =
          row['Member Price'] !== undefined
            ? parseFloat(row['Member Price'])
            : 0.0;
        const specialOfferPrice =
          row['Special Offer Price'] !== undefined
            ? parseFloat(row['Special Offer Price'])
            : 0.0;
        const retailDiscount =
          row['Retail Discount'] !== undefined
            ? parseFloat(row['Retail Discount'])
            : defaults.defaultRetailDiscount;
        ...
        if (existingProduct) {
          ...
          await this.repo.update(existingProduct.id, {
            ...
            hsnCode: row['HSN Code']
              ...
              : existingProduct.hsnCode,
            gstPercentage:
              row['GST Percentage'] !== undefined
                ? gstPercentage
                : existingProduct.gstPercentage,
            purchasePrice:
              row['Purchase Price'] !== undefined
                ? purchasePrice
            ...
            hospitalPrice:
              row['Hospital Price'] !== undefined
                ? hospitalPrice
                : existingProduct.hospitalPrice,
            memberPrice:
              row['Member Price'] !== undefined
                ? memberPrice
                : existingProduct.memberPrice,
            specialOfferPrice:
              row['Special Offer Price'] !== undefined
                ? specialOfferPrice
                : existingProduct.specialOfferPrice,
            retailDiscount:
              row['Retail Discount'] !== undefined
                ? retailDiscount
                : existingProduct.retailDiscount,
            roundOff,
            minStockLevel:
    ...
    const templateRows = [
      {
        ...
        'Wholesale Price': 35.0,
        'Hospital Price': 34.0,
        'Member Price': 33.0,
        'Special Offer Price': 32.0,
        'Retail Discount': 5.0,
        'Round Off': 'Yes',
        'Minimum Stock': 100,
        'Rack Location': 'A-04',
  ...
  async parseImportFile(fileBuffer: Buffer) {
    ...
    const workbook = xlsx.read(fileBuffer, { type: 'buffer' });
    console.log(
      '[DEBUG parseImportFile] Workbook sheet names:',
      workbook.SheetNames,
    );
    const sheetName = workbook.SheetNames[0];
    ...
    console.log(
      '[DEBUG parseImportFile] 2D Rows parsed from worksheet. Length:',
      rows2D.length,
    );
    ...
    const aliasDict = {
      ...
      offlineMarkup: [
        'offline markup',
        'offline markup %',
        'offline markup percent',
        'offlinemarkup',
      ],
      offlineSellingPrice: [
        'offline selling price',
        'offline selling rate',
        'offline price',
        'offlinesellingprice',
        'offline rate',
      ],
        ...
        for (const aliases of Object.values(aliasDict)) {
          if (
            aliases.some((alias) => {
              ...
              return (
                cleanVal === cleanAlias ||
                cleanVal.includes(cleanAlias) ||
                cleanAlias.includes(cleanVal)
              );
            })
          ) {
            matches++;
    ...
    if (headers.length === 0) {
      throw new BadRequestException(
        'Could not detect any headers in the uploaded file.',
      );
    }
  ...
  async validateImportRows(
    ...
    const rawNameKey = Object.keys(mapping).find(
      (key) => mapping[key] === 'name',
    );
    const rawBarcodeKey = Object.keys(mapping).find(
      (key) => mapping[key] === 'barcode',
    );
    const rawSkuKey = Object.keys(mapping).find(
      (key) => mapping[key] === 'sku',
    );

    const barcodes = rows
      .map((r) => (rawBarcodeKey ? String(r[rawBarcodeKey] || '').trim() : ''))
      .filter(Boolean);
    const skus = rows
      .map((r) => (rawSkuKey ? String(r[rawSkuKey] || '').trim() : ''))
      .filter(Boolean);
    const names = rows
      .map((r) => (rawNameKey ? String(r[rawNameKey] || '').trim() : ''))
      .filter(Boolean);
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const defaults = await this.getPricingDefaults();
      const offlineMarkup =
        mapped.offlineMarkup !== null &&
        mapped.offlineMarkup !== undefined &&
        mapped.offlineMarkup !== ''
          ? parseFloat(mapped.offlineMarkup)
          : defaults.defaultOfflineMarkup;
      const onlineMarkup =
        mapped.onlineMarkup !== null &&
        mapped.onlineMarkup !== undefined &&
        mapped.onlineMarkup !== ''
          ? parseFloat(mapped.onlineMarkup)
          : defaults.defaultOnlineMarkup;
      const roundOff =
        mapped.roundOff !== null &&
        mapped.roundOff !== undefined &&
        mapped.roundOff !== ''
          ? String(mapped.roundOff).toLowerCase() === 'yes' ||
      ...
      if (!isNaN(cost) && cost > 0) {
        ...
        if (onlineAutoCalculate) {
          ...
          calculatedOnlinePrice = roundOff
            ? Math.round(calc)
            : parseFloat(calc.toFixed(2));
        } else {
      ...
      if (
        mapped.wholesalePrice !== null &&
        mapped.wholesalePrice !== undefined &&
        mapped.wholesalePrice !== ''
      ) {
        if (
          isNaN(parseFloat(mapped.wholesalePrice)) ||
          parseFloat(mapped.wholesalePrice) < 0
        ) {
          rowErrors.push('Wholesale Price must be a non-negative number.');
        }
      }
      if (
        mapped.hospitalPrice !== null &&
        mapped.hospitalPrice !== undefined &&
        mapped.hospitalPrice !== ''
      ) {
        if (
          isNaN(parseFloat(mapped.hospitalPrice)) ||
          parseFloat(mapped.hospitalPrice) < 0
        ) {
          rowErrors.push('Hospital Price must be a non-negative number.');
        }
      }
      if (
        mapped.memberPrice !== null &&
        mapped.memberPrice !== undefined &&
        mapped.memberPrice !== ''
      ) {
        if (
          isNaN(parseFloat(mapped.memberPrice)) ||
          parseFloat(mapped.memberPrice) < 0
        ) {
          rowErrors.push('Member Price must be a non-negative number.');
        }
      }
      if (
        mapped.specialOfferPrice !== null &&
        mapped.specialOfferPrice !== undefined &&
        mapped.specialOfferPrice !== ''
      ) {
        if (
          isNaN(parseFloat(mapped.specialOfferPrice)) ||
          parseFloat(mapped.specialOfferPrice) < 0
        ) {
          rowErrors.push('Special Offer Price must be a non-negative number.');
        }
      }
      if (
        mapped.retailDiscount !== null &&
        mapped.retailDiscount !== undefined &&
        mapped.retailDiscount !== ''
      ) {
      ...
      if (
        mapped.gstPercentage !== null &&
        mapped.gstPercentage !== undefined &&
        mapped.gstPercentage !== ''
      ) {
      ...
      if (
        mapped.minStockLevel !== null &&
        mapped.minStockLevel !== undefined &&
        mapped.minStockLevel !== ''
      ) {
      ...
      if (existing) {
        ...
        if (duplicateMode === 'CREATE_NEW') {
          if (barcode && existing.barcode === barcode) {
            rowErrors.push(
              `Barcode unique constraint violated: "${barcode}" matches ${existing.name} in DB.`,
            );
          }
          if (sku && existing.sku === sku) {
            rowErrors.push(
              `SKU unique constraint violated: "${sku}" matches ${existing.name} in DB.`,
            );
          }
        }
      }
      ...
      resultRows.push({
        ...
        mrp: isNaN(mrp) ? 0 : mrp,
        offlineMarkup: isNaN(offlineMarkup)
          ? defaults.defaultOfflineMarkup
          : offlineMarkup,
        offlineSellingPrice: isNaN(calculatedOfflinePrice)
          ? 0
          : calculatedOfflinePrice,
        offlineAutoCalculate,
        onlineMarkup: isNaN(onlineMarkup)
          ? defaults.defaultOnlineMarkup
          : onlineMarkup,
        onlineSellingPrice: isNaN(calculatedOnlinePrice)
          ? 0
          : calculatedOnlinePrice,
        onlineAutoCalculate,
        wholesalePrice: isNaN(parseFloat(mapped.wholesalePrice))
          ? 0
          : parseFloat(mapped.wholesalePrice),
        hospitalPrice: isNaN(parseFloat(mapped.hospitalPrice))
          ? 0
          : parseFloat(mapped.hospitalPrice),
        memberPrice: isNaN(parseFloat(mapped.memberPrice))
          ? 0
          : parseFloat(mapped.memberPrice),
        specialOfferPrice: isNaN(parseFloat(mapped.specialOfferPrice))
          ? 0
          : parseFloat(mapped.specialOfferPrice),
        retailDiscount: isNaN(parseFloat(mapped.retailDiscount))
          ? defaults.defaultRetailDiscount
          : parseFloat(mapped.retailDiscount),
        roundOff,
        drugSchedule: String(mapped.drugSchedule || '').trim() || 'OTC',
        medicineClassification:
          String(mapped.medicineClassification || '').trim() || 'Other',
        prescriptionRequired:
          String(mapped.prescriptionRequired || '')
            .toLowerCase()
            .trim() === 'yes' ||
          String(mapped.prescriptionRequired || '')
            .toLowerCase()
            .trim() === 'true' ||
          mapped.prescriptionRequired === true,
        storageCondition:
          String(mapped.storageCondition || '').trim() || 'Room Temperature',
        controlledDrug:
          String(mapped.controlledDrug || '')
            .toLowerCase()
            .trim() === 'yes' ||
          String(mapped.controlledDrug || '')
            .toLowerCase()
            .trim() === 'true' ||
          mapped.controlledDrug === true,
        coldChainRequired:
          String(mapped.coldChainRequired || '')
            .toLowerCase()
            .trim() === 'yes' ||
          String(mapped.coldChainRequired || '')
            .toLowerCase()
            .trim() === 'true' ||
          mapped.coldChainRequired === true,
        highValueMedicine:
          String(mapped.highValueMedicine || '')
            .toLowerCase()
            .trim() === 'yes' ||
          String(mapped.highValueMedicine || '')
            .toLowerCase()
            .trim() === 'true' ||
          mapped.highValueMedicine === true,
  ...
  async importChunk(
    ...
    try {
      await this.prisma.$transaction(async (tx) => {
        for (let i = 0; i < rows.length; i++) {
          ...
          try {
            ...
            const drugSchedule =
              row.drugSchedule !== undefined
                ? String(row.drugSchedule).trim() || null
                : null;
            const medicineClassification =
              row.medicineClassification !== undefined
                ? String(row.medicineClassification).trim() || null
                : null;
            const prescriptionRequired =
              row.prescriptionRequired === true ||
              String(row.prescriptionRequired).toLowerCase() === 'true';
            const coldChainRequired =
              row.coldChainRequired === true ||
              String(row.coldChainRequired).toLowerCase() === 'true';
            const controlledDrug =
              row.controlledDrug === true ||
              String(row.controlledDrug).toLowerCase() === 'true';
            const highValueMedicine =
              row.highValueMedicine === true ||
              String(row.highValueMedicine).toLowerCase() === 'true';
            const storageCondition =
              row.storageCondition !== undefined
                ? String(row.storageCondition).trim() || null
                : null;

            const offlineMarkup =
              row.offlineMarkup !== undefined
                ? parseFloat(row.offlineMarkup)
                : 50.0;
            const offlineSellingPrice =
              row.offlineSellingPrice !== undefined
                ? parseFloat(row.offlineSellingPrice)
                : 0.0;
            const offlineAutoCalculate =
              row.offlineAutoCalculate !== undefined
                ? !!row.offlineAutoCalculate
                : true;
            ...
            const wholesalePrice =
              row.wholesalePrice !== undefined
                ? parseFloat(row.wholesalePrice)
                : 0.0;
            const hospitalPrice =
              row.hospitalPrice !== undefined
                ? parseFloat(row.hospitalPrice)
                : 0.0;
            const memberPrice =
              row.memberPrice !== undefined ? parseFloat(row.memberPrice) : 0.0;
            const specialOfferPrice =
              row.specialOfferPrice !== undefined
                ? parseFloat(row.specialOfferPrice)
                : 0.0;
            const retailDiscount =
              row.retailDiscount !== undefined
                ? parseFloat(row.retailDiscount)
                : 0.0;
            ...
            const categoryId = row.categoryName
              ? await resolveCategory(row.categoryName, tx)
              : null;
            const manufacturerId = row.manufacturerName
              ? await resolveManufacturer(row.manufacturerName, tx)
              : null;
            const supplierId = row.supplierName
              ? await resolveSupplier(row.supplierName, tx)
              : null;
            ...
            if (existingProduct) {
              ...
              if (duplicateMode === 'UPDATE_EXISTING') {
                  ...
                  data: {
                    ...
                    drugSchedule:
                      drugSchedule !== undefined
                        ? drugSchedule
                        : existingProduct.drugSchedule,
                    medicineClassification:
                      medicineClassification !== undefined
                        ? medicineClassification
                        : existingProduct.medicineClassification,
                    prescriptionRequired:
                      prescriptionRequired !== undefined
                        ? prescriptionRequired
                        : existingProduct.prescriptionRequired,
                    coldChainRequired:
                      coldChainRequired !== undefined
                        ? coldChainRequired
                        : existingProduct.coldChainRequired,
                    controlledDrug:
                      controlledDrug !== undefined
                        ? controlledDrug
                        : existingProduct.controlledDrug,
                    highValueMedicine:
                      highValueMedicine !== undefined
                        ? highValueMedicine
                        : existingProduct.highValueMedicine,
                    storageCondition:
                      storageCondition !== undefined
                        ? storageCondition
                        : existingProduct.storageCondition,
                    status,
                    syncStatus: 'PENDING',
                    updatedAt: new Date(),
                  },
                ...
                if (!existingProduct.genericName && genericName)
                  updateData.genericName = genericName;
                if (!existingProduct.brandName && brandName)
                ...
                if (
                  (existingProduct.gstPercentage === null ||
                    existingProduct.gstPercentage === 0) &&
                  gstPercentage
                ) {
                ...
                if (
                  (existingProduct.mrp === null || existingProduct.mrp === 0) &&
                  mrp
                ) {
                  updateData.mrp = mrp;
                }
                if (
                  (existingProduct.offlineSellingPrice === null ||
                    existingProduct.offlineSellingPrice === 0) &&
                  offlineSellingPrice
                ) {
                ...
                if (
                  (existingProduct.onlineSellingPrice === null ||
                    existingProduct.onlineSellingPrice === 0) &&
                  onlineSellingPrice
                ) {
                ...
                if (
                  (existingProduct.wholesalePrice === null ||
                ...
                if (
                  (existingProduct.hospitalPrice === null ||
                    existingProduct.hospitalPrice === 0) &&
                  hospitalPrice
                ) {
                  updateData.hospitalPrice = hospitalPrice;
                }
                if (
                  (existingProduct.memberPrice === null ||
                    existingProduct.memberPrice === 0) &&
                  memberPrice
                ) {
                  updateData.memberPrice = memberPrice;
                }
                if (
                  (existingProduct.specialOfferPrice === null ||
                    existingProduct.specialOfferPrice === 0) &&
                  specialOfferPrice
                ) {
                  updateData.specialOfferPrice = specialOfferPrice;
                }
                if (
                  (existingProduct.retailDiscount === null ||
                    existingProduct.retailDiscount === 0) &&
                  retailDiscount
                ) {
                ...
                if (
                  (existingProduct.minStockLevel === null ||
                    existingProduct.minStockLevel === 0) &&
                  minStockLevel
                ) {
                  updateData.minStockLevel = minStockLevel;
                }
                if (!existingProduct.rackLocation && rackLocation)
                  updateData.rackLocation = rackLocation;
                if (!existingProduct.description && description)
                  updateData.description = description;

                if (!existingProduct.drugSchedule && drugSchedule)
                  updateData.drugSchedule = drugSchedule;
                if (
                  !existingProduct.medicineClassification &&
                  medicineClassification
                )
                  updateData.medicineClassification = medicineClassification;
                if (
                  !existingProduct.prescriptionRequired &&
                  prescriptionRequired
                )
                  updateData.prescriptionRequired = prescriptionRequired;
                if (!existingProduct.coldChainRequired && coldChainRequired)
                  updateData.coldChainRequired = coldChainRequired;
                if (!existingProduct.controlledDrug && controlledDrug)
                  updateData.controlledDrug = controlledDrug;
                if (!existingProduct.highValueMedicine && highValueMedicine)
                  updateData.highValueMedicine = highValueMedicine;
                if (!existingProduct.storageCondition && storageCondition)
      ...
      console.warn(
        'Prisma transaction failed, falling back to sequential imports:',
        txErr.message,
      );
      successCount = 0;
      errorCount = 0;
  ...
  async emitImportStarted(
    importId: string,
    supplierName: string,
    totalRows: number,
  ) {
    this.eventEmitter.emit('product.import.started', {
      importId,
      supplierName,
      totalRows,
      startedAt: new Date(),
  ...
  async emitImportCompleted(
    ...
    this.eventEmitter.emit('product.import.completed', {
      importId,
      supplierName,
      successCount,
      errorCount,
  ...
  async emitImportFailed(
    ...
    this.eventEmitter.emit('product.import.failed', {
      importId,
      supplierName,
      error,
      failedAt: new Date(),
